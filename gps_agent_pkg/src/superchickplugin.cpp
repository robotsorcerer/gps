...
bool GPSSuperchickPlugin::init(ros::NodeHandle& n)
{
    ...
    // Variables.
    std::string base_name, head_name, right_name;
    ...
    const robot_state::JointModelGroup* joint_model_group = \
                                RobotModel->getJointModelGroup("base_bladder");
    ...
    if(!n.getParam("/GPSSuperchickPlugin/base_name", base_name)) {
        ROS_ERROR("Property base_name not found in namespace: '%s'", n.getNamespace().c_str());
        return false;
    }

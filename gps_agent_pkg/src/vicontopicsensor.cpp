...
// Constructor.
ViconTopicSensor::ViconTopicSensor(ros::NodeHandle& n, RobotPlugin *plugin): Sensor(n, plugin)
{
    // Initialize subscribers
    if (!n.getParam("feat_topic",topic_name_))
	topic_name_ = "/vicon_features_publisher";
    ...
    subscriber_ = n.subscribe(topic_name_, 1, &ViconTopicSensor::update_data_vector, this);
}
// Destructor.
ViconTopicSensor::~ViconTopicSensor()
{
    // Nothing to do here.
}
// Callback from ros topic sensor
void ViconTopicSensor::update_data_vector(const std_msgs::Float64MultiArray::ConstPtr& msg) {
...
void ViconTopicSensor::update(RobotPlugin *plugin, ros::Time current_time, bool is_controller_step)
{
    // Not needed for this sensor, since callback function is used.
}
// The settings include the configuration for the Kalman filter.
void ViconTopicSensor::configure_sensor(OptionsMap &options)
{
    ROS_INFO("configuring Vicontopicsensor");
    //data_size_ = boost::get<int>(options["data_size"]); // Maybe just set this my size of first sample?
}
// Set data format and meta data on the provided sample.
void ViconTopicSensor::set_sample_data_format(boost::scoped_ptr<Sample>& sample) 
{
...
void ViconTopicSensor::set_sample_data(boost::scoped_ptr<Sample>& sample, int t) 
{

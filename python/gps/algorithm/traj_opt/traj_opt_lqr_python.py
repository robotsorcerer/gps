...
class TrajOptLQRPython(TrajOpt):
    ...
    def update_robust(self, m, algorithm):
        ...
        for itr in range(max_itr):
            if not self.cons_per_step:
                LOGGER.debug("Robust Iteration %d, bracket: (%.2e , %.2e , %.2e)", itr,
                             min_eta, eta, max_eta)

            # Run fwd/bwd pass, note that eta may be updated.
            # Compute KL divergence constraint violation.
            traj_distr, eta = self.backward_robust(prev_traj_distr, traj_info,
                                            eta, algorithm, m)
        ...
        if (self.cons_per_step and not self._conv_check(con, kl_step)):
            ...
            for itr in range(DGD_MAX_GD_ITER):
                ...
                if not self._use_prev_distr:
                    new_mu, new_sigma = self.forward_robust(traj_distr, traj_info)
                    kl_div = traj_distr_kl(
                            new_mu, new_sigma, traj_distr, prev_traj_distr,
                            tot=False
                    )
                else:
                    prev_mu, prev_sigma = self.forward_robust(prev_traj_distr,
                                                       traj_info)
                    kl_div = traj_distr_kl_alt(
                            prev_mu, prev_sigma, traj_distr, prev_traj_distr,
                            tot=False
    ...
    def forward_robust(self, traj_distr, traj_info):
        ...
        # Set initial covariance (initial mu is always zero).
        # See algorithm.py#L160 for xomu
        sigma_u[0, idx_x, idx_x] = traj_info.x0sigma
        mu_u[0, idx_x] = traj_info.x0mu
    ...
    def backward_robust(self, prev_traj_distr, traj_info, eta, algorithm, m):
        ...
        while fail:
            ...
            for t in range(T - 1, -1, -1):
                ...
                inv_term = np.linalg.pinv(inv_term)

                # compute the inverse term for gv and Gv
                inv_term_v = PSig_v[t, :, :] - PSig_v[t, :, :].dot(PSig_v[t, :, :].dot(Qtt[t, idx_v, idx_u] \
                                ).dot(PSig_u[t, :, :]).dot(Qtt[t, idx_u, idx_v].T))
                inv_term_v = np.linalg.pinv(inv_term_v)

                if not self.cons_per_step:
                    # invert inv_term now via solve_triangular
                    gu_term = inv_term.dot(Qtt[t, idx_u, idx_v].dot(PSig_v[t,:,:]).dot(Qt[t, idx_v]) - \
                                        Qt[t, idx_u])
                    Gu_term = inv_term.dot(Qtt[t, idx_u, idx_v].dot(PSig_v[t,:,:]).dot(Qtt[t, idx_v, idx_x]) - \
                                        Qtt[t, idx_u, idx_x])

                    gv_term = inv_term_v.dot(Qtt[t, idx_v, idx_u].dot(PSig_u[t,:,:]).dot(Qt[t, idx_u]) - \
                                        Qt[t, idx_v])
                    Gv_term = inv_term_v.dot(Qtt[t, idx_v, idx_u].dot(PSig_u[t,:,:]).dot(Qtt[t, idx_u, idx_x]) - \
                                        Qtt[t, idx_v, idx_x])
                else:
                    # invert inv_term now via solve_triangular
                    ...
                    gu_term = (1.0 / eta[t]) * inv_term.dot(Qtt[t, idx_u, idx_v].dot(PSig_v[t,:,:]).dot(Qt[t, idx_v]) - \
                                        Qt[t, idx_u])
                    Gu_term = (1.0 / eta[t]) * inv_term.dot(Qtt[t, idx_u, idx_v].dot(PSig_v[t,:,:]).dot(Qtt[t, idx_v, idx_x]) - \
                                        Qtt[t, idx_u, idx_x])

                    gv_term = (1.0 / eta[t]) * inv_term_v.dot(Qtt[t, idx_v, idx_u].dot(PSig_u[t,:,:]).dot(Qt[t, idx_u]) - \
                                        Qt[t, idx_v])
                    Gv_term = (1.0 / eta[t]) * inv_term_v.dot(Qtt[t, idx_v, idx_u].dot(PSig_u[t,:,:]).dot(Qtt[t, idx_u, idx_x]) - \
                                        Qtt[t, idx_v, idx_x])
                ...
                if self._hyperparams['update_in_bwd_pass']:
                    ...
                    traj_distr.chol_pol_covar_u[t, :, :] = sp.linalg.cholesky( traj_distr.pol_covar_u[t, :, :] )
                    traj_distr.chol_pol_covar_v[t, :, :] = sp.linalg.cholesky( traj_distr.pol_covar_v[t, :, :] )

                    # Compute mean terms.
                    traj_distr.gu[t, :]     = gu_term
                    traj_distr.Gu[t, :, :]  = Gu_term
                    traj_distr.gv[t, :]     = gv_term
                    traj_distr.Gv[t, :, :]  = Gv_term
                else:
                    # Store conditional covariance, inverse, and Cholesky.
                    new_ipS[t, :, :] = inv_term
                    ...
                    new_gu[t, :] = sp.linalg.solve_triangular(
                                        U_u, sp.linalg.solve_triangular(L_u, gu_term, lower=True))
                    new_Gu[t, :, :] = sp.linalg.solve_triangular(
                                        U_u, sp.linalg.solve_triangular(L_u, Gu_term, lower=True))

                    new_gv[t, :] = sp.linalg.solve_triangular(
                                        U_v, sp.linalg.solve_triangular(L_v, gv_term, lower=True) )
                    new_Gv[t, :, :] = sp.linalg.solve_triangular(
                                        U_v, sp.linalg.solve_triangular(L_v, Gv_term, lower=True) )
                ...
                if (self.cons_per_step or \
                    ...
                    Vx[t, :] =  Qt[t, idx_x] + \
                                traj_distr.Gu[t, :, :].T.dot(Qt[t, idx_u]) + \
                                traj_distr.Gv[t, :, :].T.dot(Qt[t, idx_v]) + \
                                traj_distr.gu[t, :].T.dot(Qtt[t, idx_u, idx_u]).dot(traj_distr.Gu[t, :, :]) + \
                                traj_distr.gv[t, :].T.dot(Qtt[t, idx_v, idx_v]).dot(traj_distr.Gv[t, :, :]) + \
                                traj_distr.gu[t, :].dot(Qtt[t, idx_u, idx_x]) + \
                                traj_distr.gv[t, :].dot(Qtt[t, idx_v, idx_x]) + \
                                traj_distr.gu[t, :].dot(Qtt[t, idx_u, idx_v]).dot(traj_distr.Gv[t, :, :]) + \
                                traj_distr.gv[t, :].dot(Qtt[t, idx_v, idx_u]).dot(traj_distr.Gu[t, :, :])
                else:
                    Vxx[t, :, :] =  Qtt[t, idx_x, idx_x] + \
                                    traj_distr.Gu[t, :, :].T.dot(Qtt[t, idx_u, idx_u]).dot(traj_distr.Gu[t, :, :]) + \
                                    traj_distr.Gv[t, :, :].T.dot(Qtt[t, idx_v, idx_v]).dot(traj_distr.Gv[t, :, :]) + \
